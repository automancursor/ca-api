...
export default function RptDingdan() {
  ...
  function getAllCategory(id, token) {
    ...
    axios
      .get(BASE_URL + "/Order/rptProduct_order", config)
      .then((response) => {
  ...
  return (
    <>
      <FuncHeader title={"积分商城订单"} goBack={() => history.push("/user")} />
      <div style={{ marginTop: "50px" }}>
        {allOrders.map((order) => (
          <Cell
            ...
            onClick={() => history.push(`/rptdingdanxiangqing?id=${order?.id}`)}
          />
        ))}
      </div>
    </>
