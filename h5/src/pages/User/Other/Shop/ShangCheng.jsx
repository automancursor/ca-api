...
import ShangChengHeader from "components/FuncHeader/ShangChengHeader";
...
export default function ShangCheng() {
  ...
  function getAllCategory(id, token) {
    ...
    axios
      .get(BASE_URL + "/shop/product", config)
      .then((response) => {
  ...
  return (
    <>
      <ShangChengHeader title={"商铺管理"} goBack={() => history.push("/user")} />
      <div style={{ marginTop: "50px" }}>
        {allOrders.map((allOrders) => (
          <Cell
            ...
            onClick={() =>
              history.push("/shangchengxiangqing?itemId=" + allOrders?.id)
            }
          />
        ))}
      </div>
