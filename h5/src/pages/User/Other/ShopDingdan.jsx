...
export default function Dingdan() {
  ...
  function getAllCategory(id, token) {
    ...
    axios
      .get(BASE_URL + "/Order/product_order/seller", config)
      .then((response) => {
  ...
  return (
    <>
      <FuncHeader title={"商城订单"} goBack={() => history.push("/user")} />
      <div style={{ marginTop: "50px" }}>
        {allOrders.map((order) => (
          <Cell
            ...
            title={
              <div className="box">
                <div className="box-title">买家: {order?.userName}</div>
            ...
            description={
              order?.productOrderStatus == "PENDING" ? "进行中" : "已完成"
            }
            onClick={() =>
              history.push(`/shopdingdanxiangqing?id=${order?.id}`)
            }
          />
        ))}
      </div>
