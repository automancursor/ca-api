...
class Certification extends Component {
  ...
  getUserVerification = () => {
    ...
    axios.get(API+"/UserProfile/user_verification/all", config).then(response => {
  ...
  approveUserVerification = (id,name,) => {
    if (window.confirm("是否确认批准"+name+"的实名认证。")) {
      const user = JSON.parse(localStorage.getItem('authUser'));
      const config = {
        headers: { Authorization: `Bearer ${user.token}` }
      };
      ...
      axios.post(API+"/UserProfile/user_verification/approve/"+id, body, config).then(response => {
        if (response.status === 400 || response.status === 500)
          throw response.data;
        window.confirm("成功。")
  ...
  removeUserVerification = (id,name,) => {
    if (window.confirm("是否确认删除"+name+"的实名认证。")) {
      const user = JSON.parse(localStorage.getItem('authUser'));
      ...
      var config = {
        method: 'delete',
        url: API+'/UserProfile/user_verification/'+id,
        headers: { Authorization: `Bearer ${user.token}` },
        data : data
      };
                          ...
                          this.state.allUserVerification ?
                            this.state.allUserVerification.map((userVerificationData, key) =>
                              <tr key={key}>
                                ...
                                <td>
                                  {
                                    userVerificationData?.idCard ?
                                      <Link onClick={()=>this.viewAttachedFile(userVerificationData?.userName, userVerificationData?.idCard)} className="mr-3 text-primary">查看附件</Link> : null
                                  }
                                </td>
                                <td>
                                  {
                                    userVerificationData?.passport ?
                                      <Link onClick={()=>this.viewAttachedFile(userVerificationData?.userName, userVerificationData?.passport)} className="mr-3 text-primary">查看附件</Link> : null
                                  }
                                </td>
                                <td>{userVerificationData?.firstName}</td>
                                <td>{userVerificationData?.lastName}</td>
                                <td>{userVerificationData?.address}</td>
                                <td>
                                  {typeSwitch(userVerificationData?.userVerificationStatus)}
                                </td>
                                <td>
                                  {userVerificationData?.userVerificationStatus === "IN_PROGRESS" ? <Link className="mr-3 text-success" onClick={()=>this.approveUserVerification(userVerificationData?.id, userVerificationData?.userName)}>通过审核</Link> : null}
