// <auto-generated />
using System;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;
using cva_api.Data;

namespace cva_api.Migrations
{
    [DbContext(typeof(SqlContext))]
    [Migration("20210821132735_addRptProductOrders")]
    partial class addRptProductOrders
    {
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "3.1.12")
                .HasAnnotation("Relational:MaxIdentifierLength", 64);

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityRole", b =>
                {
                    b.Property<string>("Id")
                        .HasColumnType("varchar(255) CHARACTER SET utf8mb4");

                    b.Property<string>("ConcurrencyStamp")
                        .IsConcurrencyToken()
                        .HasColumnType("longtext CHARACTER SET utf8mb4");

                    b.Property<string>("Name")
                        .HasColumnType("varchar(256) CHARACTER SET utf8mb4")
                        .HasMaxLength(256);

                    b.Property<string>("NormalizedName")
                        .HasColumnType("varchar(256) CHARACTER SET utf8mb4")
                        .HasMaxLength(256);

                    b.HasKey("Id");

                    b.HasIndex("NormalizedName")
                        .IsUnique()
                        .HasName("RoleNameIndex");

                    b.ToTable("AspNetRoles");

                    b.HasData(
                        new
                        {
                            Id = "2301D884-221A-4E7D-B509-0113DCC043E1",
                            ConcurrencyStamp = "a712a5a0-afc7-4af8-ad2a-bb88b44cf6ff",
                            Name = "Admin",
                            NormalizedName = "ADMIN"
                        },
                        new
                        {
                            Id = "78A7570F-3CE5-48BA-9461-80283ED1D94D",
                            ConcurrencyStamp = "a5e7b9cb-e653-4599-9f5c-db3d7cfe82a8",
                            Name = "User",
                            NormalizedName = "USER"
                        });
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityRoleClaim<string>", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    b.Property<string>("ClaimType")
                        .HasColumnType("longtext CHARACTER SET utf8mb4");

                    b.Property<string>("ClaimValue")
                        .HasColumnType("longtext CHARACTER SET utf8mb4");

                    b.Property<string>("RoleId")
                        .IsRequired()
                        .HasColumnType("varchar(255) CHARACTER SET utf8mb4");

                    b.HasKey("Id");

                    b.HasIndex("RoleId");

                    b.ToTable("AspNetRoleClaims");
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserClaim<string>", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    b.Property<string>("ClaimType")
                        .HasColumnType("longtext CHARACTER SET utf8mb4");

                    b.Property<string>("ClaimValue")
                        .HasColumnType("longtext CHARACTER SET utf8mb4");

                    b.Property<string>("UserId")
                        .IsRequired()
                        .HasColumnType("varchar(255) CHARACTER SET utf8mb4");

                    b.HasKey("Id");

                    b.HasIndex("UserId");

                    b.ToTable("AspNetUserClaims");
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserLogin<string>", b =>
                {
                    b.Property<string>("LoginProvider")
                        .HasColumnType("varchar(255) CHARACTER SET utf8mb4");

                    b.Property<string>("ProviderKey")
                        .HasColumnType("varchar(255) CHARACTER SET utf8mb4");

                    b.Property<string>("ProviderDisplayName")
                        .HasColumnType("longtext CHARACTER SET utf8mb4");

                    b.Property<string>("UserId")
                        .IsRequired()
                        .HasColumnType("varchar(255) CHARACTER SET utf8mb4");

                    b.HasKey("LoginProvider", "ProviderKey");

                    b.HasIndex("UserId");

                    b.ToTable("AspNetUserLogins");
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserRole<string>", b =>
                {
                    b.Property<string>("UserId")
                        .HasColumnType("varchar(255) CHARACTER SET utf8mb4");

                    b.Property<string>("RoleId")
                        .HasColumnType("varchar(255) CHARACTER SET utf8mb4");

                    b.HasKey("UserId", "RoleId");

                    b.HasIndex("RoleId");

                    b.ToTable("AspNetUserRoles");

                    b.HasData(
                        new
                        {
                            UserId = "B22698B8-42A2-4115-9631-1C2D1E2AC5F7",
                            RoleId = "2301D884-221A-4E7D-B509-0113DCC043E1"
                        });
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserToken<string>", b =>
                {
                    b.Property<string>("UserId")
                        .HasColumnType("varchar(255) CHARACTER SET utf8mb4");

                    b.Property<string>("LoginProvider")
                        .HasColumnType("varchar(255) CHARACTER SET utf8mb4");

                    b.Property<string>("Name")
                        .HasColumnType("varchar(255) CHARACTER SET utf8mb4");

                    b.Property<string>("Value")
                        .HasColumnType("longtext CHARACTER SET utf8mb4");

                    b.HasKey("UserId", "LoginProvider", "Name");

                    b.ToTable("AspNetUserTokens");
                });

            modelBuilder.Entity("cva_api.Model.AreaClaim", b =>
                {
                    b.Property<int>("ID")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    b.Property<double>("Abg")
                        .HasColumnType("double");

                    b.Property<double>("AccumulatedBonus")
                        .HasColumnType("double");

                    b.Property<double>("AccumulatedBonusCredit")
                        .HasColumnType("double");

                    b.Property<bool>("Active")
                        .HasColumnType("tinyint(1)");

                    b.Property<string>("AreaType")
                        .HasColumnType("longtext CHARACTER SET utf8mb4");

                    b.Property<double>("Cva")
                        .HasColumnType("double");

                    b.Property<double>("Cvt")
                        .HasColumnType("double");

                    b.Property<string>("Description")
                        .HasColumnType("longtext CHARACTER SET utf8mb4");

                    b.Property<int>("LastRewardedBlockNum")
                        .HasColumnType("int");

                    b.Property<string>("Name")
                        .HasColumnType("longtext CHARACTER SET utf8mb4");

                    b.Property<int>("Round")
                        .HasColumnType("int");

                    b.Property<double>("TargetBonus")
                        .HasColumnType("double");

                    b.HasKey("ID");

                    b.ToTable("AreaClaims");

                    b.HasData(
                        new
                        {
                            ID = 1,
                            Abg = 5.0,
                            AccumulatedBonus = 0.0,
                            AccumulatedBonusCredit = 0.0,
                            Active = true,
                            AreaType = "A",
                            Cva = 500.0,
                            Cvt = 50000.0,
                            LastRewardedBlockNum = 0,
                            Name = "A区",
                            Round = 5,
                            TargetBonus = 500.0
                        },
                        new
                        {
                            ID = 2,
                            Abg = 10.0,
                            AccumulatedBonus = 0.0,
                            AccumulatedBonusCredit = 0.0,
                            Active = true,
                            AreaType = "B",
                            Cva = 1000.0,
                            Cvt = 100000.0,
                            LastRewardedBlockNum = 0,
                            Name = "B区",
                            Round = 5,
                            TargetBonus = 1000.0
                        },
                        new
                        {
                            ID = 3,
                            Abg = 20.0,
                            AccumulatedBonus = 0.0,
                            AccumulatedBonusCredit = 0.0,
                            Active = true,
                            AreaType = "C",
                            Cva = 2000.0,
                            Cvt = 200000.0,
                            LastRewardedBlockNum = 0,
                            Name = "C区",
                            Round = 5,
                            TargetBonus = 2000.0
                        },
                        new
                        {
                            ID = 4,
                            Abg = 40.0,
                            AccumulatedBonus = 0.0,
                            AccumulatedBonusCredit = 0.0,
                            Active = true,
                            AreaType = "D",
                            Cva = 4000.0,
                            Cvt = 400000.0,
                            LastRewardedBlockNum = 0,
                            Name = "D区",
                            Round = 5,
                            TargetBonus = 4000.0
                        },
                        new
                        {
                            ID = 5,
                            Abg = 80.0,
                            AccumulatedBonus = 0.0,
                            AccumulatedBonusCredit = 0.0,
                            Active = true,
                            AreaType = "E",
                            Cva = 8000.0,
                            Cvt = 800000.0,
                            LastRewardedBlockNum = 0,
                            Name = "E区",
                            Round = 5,
                            TargetBonus = 8000.0
                        },
                        new
                        {
                            ID = 6,
                            Abg = 1.0,
                            AccumulatedBonus = 0.0,
                            AccumulatedBonusCredit = 0.0,
                            Active = true,
                            AreaType = "1",
                            Cva = 100.0,
                            Cvt = 10000.0,
                            LastRewardedBlockNum = 0,
                            Name = "1区",
                            Round = 5,
                            TargetBonus = 100.0
                        },
                        new
                        {
                            ID = 7,
                            Abg = 2.0,
                            AccumulatedBonus = 0.0,
                            AccumulatedBonusCredit = 0.0,
                            Active = true,
                            AreaType = "2",
                            Cva = 200.0,
                            Cvt = 20000.0,
                            LastRewardedBlockNum = 0,
                            Name = "2区",
                            Round = 5,
                            TargetBonus = 200.0
                        },
                        new
                        {
                            ID = 8,
                            Abg = 180.0,
                            AccumulatedBonus = 0.0,
                            AccumulatedBonusCredit = 0.0,
                            Active = true,
                            AreaType = "VIP",
                            Cva = 18000.0,
                            Cvt = 1550000.0,
                            LastRewardedBlockNum = 0,
                            Name = "VIP",
                            Round = 20,
                            TargetBonus = 18000.0
                        },
                        new
                        {
                            ID = 9,
                            Abg = 360.0,
                            AccumulatedBonus = 0.0,
                            AccumulatedBonusCredit = 0.0,
                            Active = true,
                            AreaType = "县代表",
                            Cva = 36000.0,
                            Cvt = 3100000.0,
                            LastRewardedBlockNum = 0,
                            Name = "F区 / 县代表",
                            Round = 40,
                            TargetBonus = 36000.0
                        },
                        new
                        {
                            ID = 10,
                            Abg = 720.0,
                            AccumulatedBonus = 0.0,
                            AccumulatedBonusCredit = 0.0,
                            Active = true,
                            AreaType = "市代表",
                            Cva = 72000.0,
                            Cvt = 6200000.0,
                            LastRewardedBlockNum = 0,
                            Name = "G区 / 市代表",
                            Round = 80,
                            TargetBonus = 72000.0
                        },
                        new
                        {
                            ID = 11,
                            Abg = 1500.0,
                            AccumulatedBonus = 0.0,
                            AccumulatedBonusCredit = 0.0,
                            Active = true,
                            AreaType = "省代表",
                            Cva = 150000.0,
                            Cvt = 12400000.0,
                            LastRewardedBlockNum = 0,
                            Name = "H区 / 省代表",
                            Round = 160,
                            TargetBonus = 150000.0
                        },
                        new
                        {
                            ID = 12,
                            Abg = 3000.0,
                            AccumulatedBonus = 0.0,
                            AccumulatedBonusCredit = 0.0,
                            Active = true,
                            AreaType = "国家代表",
                            Cva = 300000.0,
                            Cvt = 24800000.0,
                            LastRewardedBlockNum = 0,
                            Name = "I区 / 国家代表",
                            Round = 320,
                            TargetBonus = 300000.0
                        },
                        new
                        {
                            ID = 13,
                            Abg = 6000.0,
                            AccumulatedBonus = 0.0,
                            AccumulatedBonusCredit = 0.0,
                            Active = true,
                            AreaType = "洲域代表(全球七大洲)",
                            Cva = 600000.0,
                            Cvt = 49600000.0,
                            LastRewardedBlockNum = 0,
                            Name = "K区 / 洲域代表(全球七大洲)",
                            Round = 640,
                            TargetBonus = 600000.0
                        });
                });

            modelBuilder.Entity("cva_api.Model.AreaHistory", b =>
                {
                    b.Property<int>("ID")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    b.Property<decimal>("AfterValue")
                        .HasColumnType("decimal(20, 3)");

                    b.Property<decimal>("BeforeValue")
                        .HasColumnType("decimal(20, 3)");

                    b.Property<decimal>("ChangedValue")
                        .HasColumnType("decimal(20, 3)");

                    b.Property<DateTime>("CreatedDate")
                        .HasColumnType("datetime(6)");

                    b.Property<string>("Msg")
                        .HasColumnType("longtext CHARACTER SET utf8mb4");

                    b.Property<int>("OrderId")
                        .HasColumnType("int");

                    b.Property<int>("SourceAreaRecordId")
                        .HasColumnType("int");

                    b.Property<int>("ToAreaRecordId")
                        .HasColumnType("int");

                    b.Property<string>("UpdateType")
                        .HasColumnType("longtext CHARACTER SET utf8mb4");

                    b.HasKey("ID");

                    b.ToTable("AreaHistories");
                });

            modelBuilder.Entity("cva_api.Model.AreaRecord", b =>
                {
                    b.Property<int>("ID")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    b.Property<int>("AreaClaimId")
                        .HasColumnType("int");

                    b.Property<int>("BlockNum")
                        .HasColumnType("int");

                    b.Property<string>("HistoryRecords")
                        .HasColumnType("text");

                    b.Property<string>("Lights")
                        .HasColumnType("longtext");

                    b.Property<int>("Live")
                        .HasColumnType("int");

                    b.Property<int>("OrderId")
                        .HasColumnType("int");

                    b.Property<int>("RecordIndex")
                        .HasColumnType("int");

                    b.Property<int>("RoundNum")
                        .HasColumnType("int");

                    b.Property<string>("Type")
                        .HasColumnType("longtext CHARACTER SET utf8mb4");

                    b.Property<string>("UserId")
                        .HasColumnType("varchar(255) CHARACTER SET utf8mb4");

                    b.HasKey("ID");

                    b.HasIndex("AreaClaimId");

                    b.HasIndex("OrderId")
                        .IsUnique();

                    b.HasIndex("UserId");

                    b.ToTable("AreaRecords");
                });

            modelBuilder.Entity("cva_api.Model.Category", b =>
                {
                    b.Property<int>("ID")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    b.Property<string>("CategoryImage")
                        .HasColumnType("longtext CHARACTER SET utf8mb4");

                    b.Property<string>("Description")
                        .HasColumnType("longtext CHARACTER SET utf8mb4");

                    b.Property<string>("Name")
                        .HasColumnType("longtext CHARACTER SET utf8mb4");

                    b.HasKey("ID");

                    b.ToTable("Categories");
                });

            modelBuilder.Entity("cva_api.Model.Config", b =>
                {
                    b.Property<string>("Name")
                        .HasColumnType("varchar(255) CHARACTER SET utf8mb4");

                    b.Property<string>("Description")
                        .HasColumnType("longtext CHARACTER SET utf8mb4");

                    b.Property<string>("Value")
                        .HasColumnType("longtext CHARACTER SET utf8mb4");

                    b.HasKey("Name");

                    b.ToTable("Configs");

                    b.HasData(
                        new
                        {
                            Name = "RED_REWARD",
                            Description = "红灯奖励",
                            Value = "0.06"
                        },
                        new
                        {
                            Name = "GREEN_REWARD",
                            Description = "绿灯奖励",
                            Value = "0.09"
                        },
                        new
                        {
                            Name = "GRAND_REWARD",
                            Description = "上级奖励",
                            Value = "0.03"
                        },
                        new
                        {
                            Name = "BASE_REWARD",
                            Description = "基础奖励",
                            Value = "0.03"
                        },
                        new
                        {
                            Name = "REWARD_SURCHARGE",
                            Description = "CVA转帐手续费",
                            Value = "0.1"
                        },
                        new
                        {
                            Name = "TRANSACTION_SURCHARGE",
                            Description = "CVT转帐手续费",
                            Value = "0.1"
                        },
                        new
                        {
                            Name = "WITHDRAW_SURCHARGE",
                            Description = "提现手续费",
                            Value = "0.05"
                        },
                        new
                        {
                            Name = "RECHARGE_SURCHARGE",
                            Description = "充值手续费",
                            Value = "0.1"
                        },
                        new
                        {
                            Name = "CLAIM_SURCHARGE",
                            Description = "报销手续费",
                            Value = "0.1"
                        },
                        new
                        {
                            Name = "REBORN_LIGHTS",
                            Description = "重生所需灯数",
                            Value = "12"
                        },
                        new
                        {
                            Name = "REORDER_CONTROL",
                            Description = "重复购买开关",
                            Value = "off"
                        },
                        new
                        {
                            Name = "BONUS_LOGIC",
                            Description = "衡量点产生逻辑",
                            Value = "{\"1\":{\"1\":0.125,\"2\":0.03166,\"6\":0.04,\"7\":0.05},\"2\":{\"1\":0.0417,\"2\":0.125,\"3\":0.03216,\"6\":0.0143,\"7\":0.0335},\"3\":{\"1\":0.0278,\"2\":0.0417,\"3\":0.125,\"4\":0.02786,\"6\":0.01,\"7\":0.0143},\"4\":{\"1\":0.0179,\"2\":0.025,\"3\":0.0358,\"4\":0.125,\"5\":0.03156,\"6\":0.042,\"7\":0.072},\"5\":{\"1\":0.01066,\"2\":0.018,\"3\":0.0359,\"4\":0.05,\"5\":0.125,\"6\":0.021,\"7\":0.05},\"6\":{\"6\":0.125,\"7\":0.121666},\"7\":{\"1\":0.0383,\"6\":0.125,\"7\":0.08336}}"
                        });
                });

            modelBuilder.Entity("cva_api.Model.MediaConfig", b =>
                {
                    b.Property<string>("Name")
                        .HasColumnType("varchar(255) CHARACTER SET utf8mb4");

                    b.Property<string>("Description")
                        .HasColumnType("longtext CHARACTER SET utf8mb4");

                    b.Property<string>("Value")
                        .HasColumnType("longtext CHARACTER SET utf8mb4");

                    b.HasKey("Name");

                    b.ToTable("MediaConfigs");
                });

            modelBuilder.Entity("cva_api.Model.Order", b =>
                {
                    b.Property<int>("ID")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    b.Property<double>("Abg")
                        .HasColumnType("double");

                    b.Property<DateTime>("CreatedDate")
                        .HasColumnType("datetime(6)");

                    b.Property<double>("Cva")
                        .HasColumnType("double");

                    b.Property<double>("Cvt")
                        .HasColumnType("double");

                    b.Property<double>("CvtCredit")
                        .HasColumnType("double");

                    b.Property<string>("Msg")
                        .HasColumnType("longtext CHARACTER SET utf8mb4");

                    b.Property<string>("UserId")
                        .HasColumnType("varchar(255) CHARACTER SET utf8mb4");

                    b.HasKey("ID");

                    b.HasIndex("UserId");

                    b.ToTable("Orders");
                });

            modelBuilder.Entity("cva_api.Model.Product", b =>
                {
                    b.Property<int>("ID")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    b.Property<string>("Address")
                        .HasColumnType("longtext CHARACTER SET utf8mb4");

                    b.Property<int>("CategoryId")
                        .HasColumnType("int");

                    b.Property<string>("CompanyName")
                        .HasColumnType("longtext CHARACTER SET utf8mb4");

                    b.Property<DateTime?>("CreatedDate")
                        .HasColumnType("datetime(6)");

                    b.Property<double>("Cvt")
                        .HasColumnType("double");

                    b.Property<double>("CvtPrice")
                        .HasColumnType("double");

                    b.Property<string>("Name")
                        .HasColumnType("longtext CHARACTER SET utf8mb4");

                    b.Property<string>("ProductDescription")
                        .HasColumnType("longtext CHARACTER SET utf8mb4");

                    b.Property<string>("ProductImages")
                        .HasColumnType("longtext CHARACTER SET utf8mb4");

                    b.Property<int>("ProductStatus")
                        .HasColumnType("int");

                    b.Property<int>("Quantity")
                        .HasColumnType("int");

                    b.Property<string>("UserId")
                        .HasColumnType("varchar(255) CHARACTER SET utf8mb4");

                    b.Property<string>("UserName")
                        .HasColumnType("longtext CHARACTER SET utf8mb4");

                    b.HasKey("ID");

                    b.HasIndex("CategoryId");

                    b.HasIndex("UserId");

                    b.ToTable("Products");
                });

            modelBuilder.Entity("cva_api.Model.ProductOrder", b =>
                {
                    b.Property<int>("ID")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    b.Property<string>("Address")
                        .HasColumnType("longtext CHARACTER SET utf8mb4");

                    b.Property<DateTime?>("CreatedDate")
                        .HasColumnType("datetime(6)");

                    b.Property<double>("Cvt")
                        .HasColumnType("double");

                    b.Property<string>("OrderDetail")
                        .HasColumnType("longtext CHARACTER SET utf8mb4");

                    b.Property<string>("OrderProducts")
                        .HasColumnType("text");

                    b.Property<int>("ProductOrderStatus")
                        .HasColumnType("int");

                    b.Property<string>("SellerId")
                        .HasColumnType("varchar(255) CHARACTER SET utf8mb4");

                    b.Property<string>("SellerName")
                        .HasColumnType("longtext CHARACTER SET utf8mb4");

                    b.Property<string>("UserId")
                        .HasColumnType("varchar(255) CHARACTER SET utf8mb4");

                    b.Property<string>("UserName")
                        .HasColumnType("longtext CHARACTER SET utf8mb4");

                    b.HasKey("ID");

                    b.HasIndex("SellerId");

                    b.HasIndex("UserId");

                    b.ToTable("ProductOrders");
                });

            modelBuilder.Entity("cva_api.Model.RptProduct", b =>
                {
                    b.Property<int>("ID")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    b.Property<string>("Address")
                        .HasColumnType("longtext CHARACTER SET utf8mb4");

                    b.Property<int>("CategoryId")
                        .HasColumnType("int");

                    b.Property<string>("CompanyName")
                        .HasColumnType("longtext CHARACTER SET utf8mb4");

                    b.Property<double>("Cost")
                        .HasColumnType("double");

                    b.Property<string>("CostCurrency")
                        .HasColumnType("longtext CHARACTER SET utf8mb4");

                    b.Property<DateTime?>("CreatedDate")
                        .HasColumnType("datetime(6)");

                    b.Property<string>("Currency")
                        .HasColumnType("longtext CHARACTER SET utf8mb4");

                    b.Property<string>("Name")
                        .HasColumnType("longtext CHARACTER SET utf8mb4");

                    b.Property<double>("Price")
                        .HasColumnType("double");

                    b.Property<string>("ProductDescription")
                        .HasColumnType("longtext CHARACTER SET utf8mb4");

                    b.Property<string>("ProductImages")
                        .HasColumnType("longtext CHARACTER SET utf8mb4");

                    b.Property<int>("ProductStatus")
                        .HasColumnType("int");

                    b.Property<int>("Quantity")
                        .HasColumnType("int");

                    b.Property<string>("UserId")
                        .HasColumnType("varchar(255) CHARACTER SET utf8mb4");

                    b.Property<string>("UserName")
                        .HasColumnType("longtext CHARACTER SET utf8mb4");

                    b.HasKey("ID");

                    b.HasIndex("CategoryId");

                    b.HasIndex("UserId");

                    b.ToTable("RptProducts");
                });

            modelBuilder.Entity("cva_api.Model.RptProductOrder", b =>
                {
                    b.Property<int>("ID")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    b.Property<string>("Address")
                        .HasColumnType("longtext CHARACTER SET utf8mb4");

                    b.Property<DateTime?>("CreatedDate")
                        .HasColumnType("datetime(6)");

                    b.Property<string>("Currency")
                        .HasColumnType("longtext CHARACTER SET utf8mb4");

                    b.Property<string>("OrderDetail")
                        .HasColumnType("longtext CHARACTER SET utf8mb4");

                    b.Property<string>("PaymentMethod")
                        .HasColumnType("longtext CHARACTER SET utf8mb4");

                    b.Property<string>("PaymentToken")
                        .HasColumnType("longtext CHARACTER SET utf8mb4");

                    b.Property<double>("Price")
                        .HasColumnType("double");

                    b.Property<string>("RptOrderProducts")
                        .HasColumnType("text");

                    b.Property<int>("RptProductOrderStatus")
                        .HasColumnType("int");

                    b.Property<string>("SellerId")
                        .HasColumnType("varchar(255) CHARACTER SET utf8mb4");

                    b.Property<string>("SellerName")
                        .HasColumnType("longtext CHARACTER SET utf8mb4");

                    b.Property<string>("UserId")
                        .HasColumnType("varchar(255) CHARACTER SET utf8mb4");

                    b.Property<string>("UserName")
                        .HasColumnType("longtext CHARACTER SET utf8mb4");

                    b.HasKey("ID");

                    b.HasIndex("SellerId");

                    b.HasIndex("UserId");

                    b.ToTable("RptProductOrders");
                });

            modelBuilder.Entity("cva_api.Model.SellerVerification", b =>
                {
                    b.Property<int>("ID")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    b.Property<string>("ABN")
                        .HasColumnType("longtext CHARACTER SET utf8mb4");

                    b.Property<string>("AvatarImage")
                        .HasColumnType("longtext CHARACTER SET utf8mb4");

                    b.Property<string>("CompanyAddress")
                        .HasColumnType("longtext CHARACTER SET utf8mb4");

                    b.Property<string>("CompanyLink")
                        .HasColumnType("longtext CHARACTER SET utf8mb4");

                    b.Property<string>("CompanyName")
                        .HasColumnType("longtext CHARACTER SET utf8mb4");

                    b.Property<string>("Country")
                        .HasColumnType("longtext CHARACTER SET utf8mb4");

                    b.Property<DateTime?>("CreatedDate")
                        .HasColumnType("datetime(6)");

                    b.Property<string>("Email")
                        .HasColumnType("longtext CHARACTER SET utf8mb4");

                    b.Property<string>("Mobile")
                        .HasColumnType("longtext CHARACTER SET utf8mb4");

                    b.Property<string>("Registration")
                        .HasColumnType("text");

                    b.Property<double?>("RptRate")
                        .HasColumnType("double");

                    b.Property<int>("SellerVerificationStatus")
                        .HasColumnType("int");

                    b.Property<string>("ShopName")
                        .HasColumnType("longtext CHARACTER SET utf8mb4");

                    b.Property<string>("UserId")
                        .HasColumnType("varchar(255) CHARACTER SET utf8mb4");

                    b.Property<string>("UserName")
                        .HasColumnType("longtext CHARACTER SET utf8mb4");

                    b.HasKey("ID");

                    b.HasIndex("UserId");

                    b.ToTable("SellerVerifications");
                });

            modelBuilder.Entity("cva_api.Model.Transaction", b =>
                {
                    b.Property<int>("ID")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    b.Property<decimal?>("Amount")
                        .HasColumnType("decimal(20, 3)");

                    b.Property<string>("AttachedFilesLink")
                        .HasColumnType("longtext CHARACTER SET utf8mb4");

                    b.Property<DateTime?>("CreatedDate")
                        .HasColumnType("datetime(6)");

                    b.Property<string>("PaymentLink")
                        .HasColumnType("longtext CHARACTER SET utf8mb4");

                    b.Property<string>("ToId")
                        .HasColumnType("longtext CHARACTER SET utf8mb4");

                    b.Property<int>("TransactionCoinType")
                        .HasColumnType("int");

                    b.Property<string>("TransactionMethod")
                        .HasColumnType("longtext CHARACTER SET utf8mb4");

                    b.Property<int>("TransactionStatus")
                        .HasColumnType("int");

                    b.Property<int>("TransactionType")
                        .HasColumnType("int");

                    b.Property<string>("UserId")
                        .HasColumnType("varchar(255) CHARACTER SET utf8mb4");

                    b.HasKey("ID");

                    b.HasIndex("UserId");

                    b.ToTable("Transactions");
                });

            modelBuilder.Entity("cva_api.Model.User", b =>
                {
                    b.Property<string>("Id")
                        .HasColumnType("varchar(255) CHARACTER SET utf8mb4");

                    b.Property<string>("Abn")
                        .HasColumnType("longtext CHARACTER SET utf8mb4");

                    b.Property<int>("AccessFailedCount")
                        .HasColumnType("int");

                    b.Property<string>("Address")
                        .HasColumnType("longtext CHARACTER SET utf8mb4");

                    b.Property<string>("AliPay")
                        .HasColumnType("longtext CHARACTER SET utf8mb4");

                    b.Property<string>("BankDetailCN")
                        .HasColumnType("longtext CHARACTER SET utf8mb4");

                    b.Property<string>("BlockChainWalletAddress")
                        .HasColumnType("longtext CHARACTER SET utf8mb4");

                    b.Property<string>("CompanyDescription")
                        .HasColumnType("longtext CHARACTER SET utf8mb4");

                    b.Property<string>("CompanyLink")
                        .HasColumnType("longtext CHARACTER SET utf8mb4");

                    b.Property<string>("CompanyName")
                        .HasColumnType("longtext CHARACTER SET utf8mb4");

                    b.Property<string>("ConcurrencyStamp")
                        .IsConcurrencyToken()
                        .HasColumnType("longtext CHARACTER SET utf8mb4");

                    b.Property<string>("CountryCode")
                        .HasColumnType("longtext CHARACTER SET utf8mb4");

                    b.Property<DateTime>("CreatedDate")
                        .HasColumnType("datetime(6)");

                    b.Property<string>("Email")
                        .HasColumnType("varchar(256) CHARACTER SET utf8mb4")
                        .HasMaxLength(256);

                    b.Property<bool>("EmailConfirmed")
                        .HasColumnType("tinyint(1)");

                    b.Property<string>("IdCard")
                        .HasColumnType("text");

                    b.Property<bool>("IsNewMember")
                        .HasColumnType("tinyint(1)");

                    b.Property<bool>("LockoutEnabled")
                        .HasColumnType("tinyint(1)");

                    b.Property<DateTimeOffset?>("LockoutEnd")
                        .HasColumnType("datetime(6)");

                    b.Property<string>("NormalizedEmail")
                        .HasColumnType("varchar(256) CHARACTER SET utf8mb4")
                        .HasMaxLength(256);

                    b.Property<string>("NormalizedUserName")
                        .HasColumnType("varchar(256) CHARACTER SET utf8mb4")
                        .HasMaxLength(256);

                    b.Property<string>("OverSeaBankDetail")
                        .HasColumnType("longtext CHARACTER SET utf8mb4");

                    b.Property<string>("Passport")
                        .HasColumnType("text");

                    b.Property<string>("PasswordHash")
                        .HasColumnType("longtext CHARACTER SET utf8mb4");

                    b.Property<string>("PayPasswordHash")
                        .HasColumnType("longtext CHARACTER SET utf8mb4");

                    b.Property<string>("PhoneNumber")
                        .HasColumnType("longtext CHARACTER SET utf8mb4");

                    b.Property<bool>("PhoneNumberConfirmed")
                        .HasColumnType("tinyint(1)");

                    b.Property<string>("ReferCode")
                        .HasColumnType("longtext CHARACTER SET utf8mb4");

                    b.Property<string>("ReferrerId")
                        .HasColumnType("varchar(255) CHARACTER SET utf8mb4");

                    b.Property<double>("RptRate")
                        .HasColumnType("double");

                    b.Property<string>("SecurityStamp")
                        .HasColumnType("longtext CHARACTER SET utf8mb4");

                    b.Property<bool>("SellerVerified")
                        .HasColumnType("tinyint(1)");

                    b.Property<string>("ShopName")
                        .HasColumnType("longtext CHARACTER SET utf8mb4");

                    b.Property<bool>("TwoFactorEnabled")
                        .HasColumnType("tinyint(1)");

                    b.Property<int>("UserIndex")
                        .HasColumnType("int");

                    b.Property<string>("UserName")
                        .HasColumnType("varchar(256) CHARACTER SET utf8mb4")
                        .HasMaxLength(256);

                    b.Property<string>("Wechat")
                        .HasColumnType("longtext CHARACTER SET utf8mb4");

                    b.Property<string>("shopAvatarImage")
                        .HasColumnType("longtext CHARACTER SET utf8mb4");

                    b.HasKey("Id");

                    b.HasIndex("NormalizedEmail")
                        .HasName("EmailIndex");

                    b.HasIndex("NormalizedUserName")
                        .IsUnique()
                        .HasName("UserNameIndex");

                    b.HasIndex("ReferrerId");

                    b.ToTable("AspNetUsers");

                    b.HasData(
                        new
                        {
                            Id = "B22698B8-42A2-4115-9631-1C2D1E2AC5F7",
                            AccessFailedCount = 0,
                            ConcurrencyStamp = "41ab8e17-b6f9-4c00-b76d-2e05638fd616",
                            CreatedDate = new DateTime(2021, 8, 21, 23, 27, 35, 59, DateTimeKind.Local).AddTicks(7750),
                            EmailConfirmed = false,
                            IsNewMember = true,
                            LockoutEnabled = false,
                            NormalizedUserName = "BBW",
                            PasswordHash = "AQAAAAEAACcQAAAAEGxnON2bxCpXEEZRKFl9uaxOKcvKAv+Iy+CWn5vIW5iKqBvJvl7P71lwGgmtcc+zTA==",
                            PayPasswordHash = "123123",
                            PhoneNumberConfirmed = false,
                            ReferCode = "00000000",
                            RptRate = 0.0,
                            SecurityStamp = "00000000-0000-0000-0000-000000000000",
                            SellerVerified = false,
                            TwoFactorEnabled = false,
                            UserIndex = 1,
                            UserName = "BBW"
                        });
                });

            modelBuilder.Entity("cva_api.Model.UserVerification", b =>
                {
                    b.Property<int>("ID")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    b.Property<string>("Address")
                        .HasColumnType("longtext CHARACTER SET utf8mb4");

                    b.Property<string>("AdminMessage")
                        .HasColumnType("longtext CHARACTER SET utf8mb4");

                    b.Property<DateTime?>("CreatedDate")
                        .HasColumnType("datetime(6)");

                    b.Property<string>("FirstName")
                        .HasColumnType("longtext CHARACTER SET utf8mb4");

                    b.Property<string>("IdCard")
                        .HasColumnType("text");

                    b.Property<string>("IdCardNumber")
                        .HasColumnType("longtext CHARACTER SET utf8mb4");

                    b.Property<string>("LastName")
                        .HasColumnType("longtext CHARACTER SET utf8mb4");

                    b.Property<string>("Passport")
                        .HasColumnType("text");

                    b.Property<string>("PassportNumber")
                        .HasColumnType("longtext CHARACTER SET utf8mb4");

                    b.Property<string>("UserId")
                        .HasColumnType("varchar(255) CHARACTER SET utf8mb4");

                    b.Property<string>("UserMessage")
                        .HasColumnType("longtext CHARACTER SET utf8mb4");

                    b.Property<string>("UserName")
                        .HasColumnType("longtext CHARACTER SET utf8mb4");

                    b.Property<int>("UserVerificationStatus")
                        .HasColumnType("int");

                    b.HasKey("ID");

                    b.HasIndex("UserId");

                    b.ToTable("UserVerifications");
                });

            modelBuilder.Entity("cva_api.Model.Wallet", b =>
                {
                    b.Property<string>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("varchar(255) CHARACTER SET utf8mb4");

                    b.Property<decimal>("Abg")
                        .HasColumnType("decimal(10, 3)");

                    b.Property<decimal>("Cva")
                        .HasColumnType("decimal(20, 3)");

                    b.Property<decimal>("Cvt")
                        .HasColumnType("decimal(20, 3)");

                    b.Property<decimal>("CvtCredit")
                        .HasColumnType("decimal(30, 3)");

                    b.Property<decimal>("Rpt")
                        .HasColumnType("decimal(10, 3)");

                    b.Property<decimal>("RptDebts")
                        .HasColumnType("decimal(10, 3)");

                    b.Property<string>("UserId")
                        .HasColumnType("varchar(255) CHARACTER SET utf8mb4");

                    b.HasKey("Id");

                    b.HasIndex("UserId")
                        .IsUnique();

                    b.ToTable("Wallet");

                    b.HasData(
                        new
                        {
                            Id = "7D9B7113-A8F8-4035-99A7-A20DD400F6A3",
                            Abg = 0m,
                            Cva = 15800m,
                            Cvt = 0m,
                            CvtCredit = 0m,
                            Rpt = 0m,
                            RptDebts = 0m,
                            UserId = "B22698B8-42A2-4115-9631-1C2D1E2AC5F7"
                        });
                });

            modelBuilder.Entity("cva_api.Model.WalletHistory", b =>
                {
                    b.Property<int>("ID")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    b.Property<decimal>("AfterValue")
                        .HasColumnType("decimal(20, 3)");

                    b.Property<decimal>("BeforeValue")
                        .HasColumnType("decimal(20, 3)");

                    b.Property<DateTime?>("CreatedDate")
                        .HasColumnType("datetime(6)");

                    b.Property<string>("Msg")
                        .HasColumnType("longtext CHARACTER SET utf8mb4");

                    b.Property<string>("UpdateType")
                        .HasColumnType("longtext CHARACTER SET utf8mb4");

                    b.Property<int>("WalletHistoryType")
                        .HasColumnType("int");

                    b.Property<string>("WalletId")
                        .HasColumnType("varchar(255) CHARACTER SET utf8mb4");

                    b.HasKey("ID");

                    b.HasIndex("WalletId");

                    b.ToTable("WalletHistories");
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityRoleClaim<string>", b =>
                {
                    b.HasOne("Microsoft.AspNetCore.Identity.IdentityRole", null)
                        .WithMany()
                        .HasForeignKey("RoleId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserClaim<string>", b =>
                {
                    b.HasOne("cva_api.Model.User", null)
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserLogin<string>", b =>
                {
                    b.HasOne("cva_api.Model.User", null)
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserRole<string>", b =>
                {
                    b.HasOne("Microsoft.AspNetCore.Identity.IdentityRole", null)
                        .WithMany()
                        .HasForeignKey("RoleId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("cva_api.Model.User", null)
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserToken<string>", b =>
                {
                    b.HasOne("cva_api.Model.User", null)
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("cva_api.Model.AreaRecord", b =>
                {
                    b.HasOne("cva_api.Model.AreaClaim", "AreaClaim")
                        .WithMany("AreaRecords")
                        .HasForeignKey("AreaClaimId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("cva_api.Model.Order", "Order")
                        .WithOne("AreaRecord")
                        .HasForeignKey("cva_api.Model.AreaRecord", "OrderId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("cva_api.Model.User", "User")
                        .WithMany("AreaRecords")
                        .HasForeignKey("UserId");
                });

            modelBuilder.Entity("cva_api.Model.Order", b =>
                {
                    b.HasOne("cva_api.Model.User", "User")
                        .WithMany("Orders")
                        .HasForeignKey("UserId");
                });

            modelBuilder.Entity("cva_api.Model.Product", b =>
                {
                    b.HasOne("cva_api.Model.Category", "Category")
                        .WithMany("Products")
                        .HasForeignKey("CategoryId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("cva_api.Model.User", "User")
                        .WithMany("Products")
                        .HasForeignKey("UserId");
                });

            modelBuilder.Entity("cva_api.Model.ProductOrder", b =>
                {
                    b.HasOne("cva_api.Model.User", "Seller")
                        .WithMany("SellProductOrders")
                        .HasForeignKey("SellerId");

                    b.HasOne("cva_api.Model.User", "User")
                        .WithMany("ProductOrders")
                        .HasForeignKey("UserId");
                });

            modelBuilder.Entity("cva_api.Model.RptProduct", b =>
                {
                    b.HasOne("cva_api.Model.Category", "Category")
                        .WithMany("RptProducts")
                        .HasForeignKey("CategoryId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("cva_api.Model.User", "User")
                        .WithMany("RptProducts")
                        .HasForeignKey("UserId");
                });

            modelBuilder.Entity("cva_api.Model.RptProductOrder", b =>
                {
                    b.HasOne("cva_api.Model.User", "Seller")
                        .WithMany("SellRptProductOrders")
                        .HasForeignKey("SellerId");

                    b.HasOne("cva_api.Model.User", "User")
                        .WithMany("RptProductOrders")
                        .HasForeignKey("UserId");
                });

            modelBuilder.Entity("cva_api.Model.SellerVerification", b =>
                {
                    b.HasOne("cva_api.Model.User", "User")
                        .WithMany("SellerVerifications")
                        .HasForeignKey("UserId");
                });

            modelBuilder.Entity("cva_api.Model.Transaction", b =>
                {
                    b.HasOne("cva_api.Model.User", "User")
                        .WithMany("Transactions")
                        .HasForeignKey("UserId");
                });

            modelBuilder.Entity("cva_api.Model.User", b =>
                {
                    b.HasOne("cva_api.Model.User", "Referrer")
                        .WithMany("Referees")
                        .HasForeignKey("ReferrerId");
                });

            modelBuilder.Entity("cva_api.Model.UserVerification", b =>
                {
                    b.HasOne("cva_api.Model.User", "User")
                        .WithMany("UserVerifications")
                        .HasForeignKey("UserId");
                });

            modelBuilder.Entity("cva_api.Model.Wallet", b =>
                {
                    b.HasOne("cva_api.Model.User", "User")
                        .WithOne("Wallet")
                        .HasForeignKey("cva_api.Model.Wallet", "UserId");
                });

            modelBuilder.Entity("cva_api.Model.WalletHistory", b =>
                {
                    b.HasOne("cva_api.Model.Wallet", "Wallet")
                        .WithMany("WalletHistories")
                        .HasForeignKey("WalletId");
                });
#pragma warning restore 612, 618
        }
    }
}
